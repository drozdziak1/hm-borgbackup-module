...
let
  ...
  mkBackupScript = cfg: ''
    ...
    export BORG=${pkgs.borgbackup}/bin/borg
  ...
  mkJobName = name: "borgbackup-job-${name}";

  mkBackupService = name: cfg:
    let
      userHome = cfg.userHome;
      scriptSrc = mkBackupScript cfg;
      script = pkgs.writeShellScriptBin "runBorg" scriptSrc;
      jobName = mkJobName name;
    in
          ...
          { name, config, ... }: {
            options = {
              ...
              userHome = mkOption {
                type = types.str;
                description = ''
                  The directory of <literal>user</literal>.
                '';
                default = "${globalConfig.home.homeDirectory}";
              };
